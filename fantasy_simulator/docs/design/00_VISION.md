...
**「에르도리아(Eldoria)」는 우리 게임의 시스템·행성·서사를 모두 대표하는 이름이다** — VR×BCI×Mnemosyne 샤드 위에서 돌아가는 **한 행성 전체**이며, 애쉬포인트 같은 마을은 그 위의 한 거점일 뿐이다. 플레이어는 전송된 이방인으로 봉인·세력·결말을 쓴다.

MMO로 확장할 때 **소수의 전설급 강자는 있되, 다수가 독점·지배층이 되지 않는** 전력 생태를 목표로 한다 → [24_ELDORIA_UNIVERSE_AND_POWER_ECOLOGY.md](24_ELDORIA_UNIVERSE_AND_POWER_ECOLOGY.md).
...
3. **행성 에르도리아층 (Diegetic)** — 봉인, 종족 영역, 왕국·마을 — 플레이어는 이 층의 언어를 기본으로 쓴다.
