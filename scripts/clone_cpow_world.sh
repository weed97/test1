#!/usr/bin/env bash
# Clone CPoW World — prefers weed97/cpow_world, falls back to test1 export branch.
set -euo pipefail
DEST="${1:-../cpow_world}"
CPOW_REPO="https://github.com/weed97/cpow_world.git"
...
if git ls-remote "$CPOW_REPO" refs/heads/main 2>/dev/null | grep -q .; then
  ...
  echo "cpow_world repo empty — cloning test1/$EXPORT_BRANCH export branch..."
